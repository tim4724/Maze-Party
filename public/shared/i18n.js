...
var LOCALES = {
  en: {
    ...
    privacy_data: 'data',
    privacy_data_detail: 'IP addresses are processed by the web server when you access the website. During a multiplayer session, they are also processed by our game server, which relays messages between the participating devices. When devices establish a direct connection to each other, a helper server (stun.couch-games.com) is additionally involved to assist with that connection (no gameplay data is transmitted through it). Web server access logs are retained for a maximum of 7 days for security and operational purposes and then automatically deleted. The game server itself stores no connection data; infrastructure-level logs by our processor are retained solely for security and operational purposes and no longer than necessary for those purposes. The helper server retains no data.',
    privacy_data_basis: 'Recipients of the connection data are the controller and the processor disclosed under "Hosting"; no further disclosure takes place. Processing is based on legitimate interest in operating the service (Art. 6(1)(f) GDPR). Providing your IP address is technically necessary to access the website and participate in the game; without this data the service cannot be used. There is no further statutory or contractual obligation to provide data.',
    privacy_tracking: 'tracking and local storage',
    privacy_tracking_detail: 'No cookies, no analytics or advertising services, and no third-party trackers are used. Settings strictly necessary for service operation within the meaning of Section 25(2) No. 2 TDDDG (German Telecommunications-Digital-Services Data Protection Act) are stored locally in the browser (e.g. the last-entered player name, chosen color, mute state, further display preferences, and a reconnection ID scoped to the current game room). Local storage is not a tracking mechanism; it merely retains these settings for the next session.',
    privacy_gameplay: 'gameplay',
    privacy_gameplay_detail: 'Player names and inputs are transmitted between players during a session and are not retained beyond the session. For lower input latency, inputs may, where technically possible, be transmitted directly between the participating devices; otherwise transmission goes via the game server (which only forwards the messages). In direct connection mode, the participating devices exchange their IP addresses for this purpose, which is technically required for any direct internet connection. Processing is based on legitimate interest in operating the service (Art. 6(1)(f) GDPR).',
    privacy_hosting: 'hosting',
    privacy_hosting_detail: 'The website and the helper server (stun.couch-games.com) are hosted on servers in Germany operated by the controller. The game server used for multiplayer (ws.couch-games.com) is operated for us by Fly.io, Inc. (Chicago, USA) as a processor under Art. 28 GDPR. Because the game server runs in multiple regions, traffic may pass through locations outside the EEA; such transfers are safeguarded by the EU Standard Contractual Clauses (Decision (EU) 2021/914, Module 2 pursuant to Art. 46(2)(c) GDPR) and supplementary technical measures (TLS encryption in transit). Fly.io, Inc. is also certified under the EU-US Data Privacy Framework (adequacy decision (EU) 2023/1795). A copy of the Standard Contractual Clauses is available on request.',
    privacy_rights: 'your rights',
    privacy_rights_detail: 'You have the right to access (Art. 15 GDPR), rectification (Art. 16), erasure (Art. 17), and restriction of processing (Art. 18). The right to data portability (Art. 20 GDPR) does not apply because processing is not based on consent or a contract. To exercise these rights, contact the controller at the e-mail address above. Your separate right to object under Art. 21 GDPR is set out below. No automated decision-making, including profiling (Art. 22 GDPR), takes place.',
    ...
    imprint_page_title: 'Imprint · Maze Party',
    privacy_page_title: 'Privacy · Maze Party',
  ...
  de: {
    ...
    imprint_page_title: 'Impressum · Maze Party',
    privacy_page_title: 'Datenschutz · Maze Party',
    back: 'Zurück',
    ...
    privacy_data: 'Daten',
    privacy_data_detail: 'IP-Adressen werden vom Webserver beim Aufruf der Website verarbeitet. Während einer Multiplayer-Sitzung werden sie zudem von unserem Spielserver verarbeitet, der Nachrichten zwischen den Geräten vermittelt. Bei direkten Verbindungen zwischen Spielgeräten kommt ein Hilfsserver (stun.couch-games.com) hinzu, der den Geräten beim Verbindungsaufbau hilft (es werden keine Spieldaten darüber übertragen). Zugriffslogs des Webservers werden zu Sicherheits- und Betriebszwecken für maximal 7 Tage gespeichert und anschließend automatisch gelöscht. Der Spielserver speichert selbst keine Verbindungsdaten; infrastrukturseitige Logs unseres Auftragsverarbeiters werden ausschließlich zu Sicherheits- und Betriebszwecken aufbewahrt und nicht länger, als für diese Zwecke erforderlich. Der Hilfsserver speichert keine Daten.',
    privacy_data_basis: 'Empfänger der Verbindungsdaten ist neben dem Verantwortlichen ausschließlich der unter „Hosting" benannte Auftragsverarbeiter; eine darüber hinausgehende Weitergabe findet nicht statt. Die Verarbeitung erfolgt auf Grundlage des berechtigten Interesses am Betrieb des Dienstes (Art. 6 Abs. 1 lit. f DSGVO). Eine Bereitstellung Ihrer IP-Adresse ist technisch notwendig, um die Website abzurufen und am Spiel teilzunehmen; ohne diese Daten ist eine Nutzung des Dienstes nicht möglich. Eine darüber hinausgehende gesetzliche oder vertragliche Pflicht zur Bereitstellung besteht nicht.',
    privacy_tracking: 'Tracking und lokale Speicherung',
    privacy_tracking_detail: 'Es werden keine Cookies, keine Analyse- oder Werbedienste und keine Drittanbieter-Tracker eingesetzt. Für den Spielbetrieb unbedingt erforderlich i.S.v. § 25 Abs. 2 Nr. 2 TDDDG ist die lokale Speicherung weniger Einstellungen im Browser des Endgeräts (z. B. zuletzt eingegebener Spielername, gewählte Spielfarbe, Stummschaltung, weitere Anzeigeeinstellungen und eine Wiederverbindungs-ID, die nur für den aktuellen Spielraum gilt). Die lokale Speicherung ist keine Tracking-Maßnahme; sie dient lediglich der Vorhaltung dieser Einstellungen für die nächste Sitzung.',
    privacy_gameplay: 'Spielbetrieb',
    privacy_gameplay_detail: 'Spielernamen und Eingaben werden während einer Sitzung zwischen den Spielern übertragen und nicht über die Sitzung hinaus gespeichert. Für niedrigere Eingabelatenz können Eingaben, sofern technisch möglich, direkt zwischen den beteiligten Geräten übertragen werden; andernfalls läuft die Übertragung über den Spielserver (der die Nachrichten lediglich weiterleitet). Beim direkten Verbindungsmodus tauschen die beteiligten Geräte hierfür ihre IP-Adressen aus, wie bei jeder direkten Internetverbindung technisch notwendig. Die Verarbeitung erfolgt auf Grundlage des berechtigten Interesses am Betrieb des Dienstes (Art. 6 Abs. 1 lit. f DSGVO).',
    privacy_hosting: 'Hosting',
    privacy_hosting_detail: 'Die Website und der Hilfsserver (stun.couch-games.com) werden auf Servern des Verantwortlichen in Deutschland gehostet. Der für Multiplayer genutzte Spielserver (ws.couch-games.com) wird für uns von Fly.io, Inc. (Chicago, USA) als Auftragsverarbeiter nach Art. 28 DSGVO betrieben. Da der Spielserver mehrregional läuft, kann Datenverkehr auch über Standorte außerhalb des EWR geroutet werden; solche Übermittlungen sind durch die EU-Standardvertragsklauseln (Beschluss (EU) 2021/914, Modul 2 nach Art. 46 Abs. 2 lit. c DSGVO) sowie ergänzende technische Maßnahmen (TLS-Verschlüsselung in Transit) abgesichert. Fly.io, Inc. ist zudem nach dem EU-US Data Privacy Framework (Angemessenheitsbeschluss (EU) 2023/1795) zertifiziert. Eine Kopie der Standardvertragsklauseln stellen wir auf Anfrage bereit.',
    privacy_rights: 'Rechte der Betroffenen',
    privacy_rights_detail: 'Sie haben das Recht auf Auskunft (Art. 15 DSGVO), Berichtigung (Art. 16), Löschung (Art. 17) sowie Einschränkung der Verarbeitung (Art. 18). Ein Recht auf Datenübertragbarkeit (Art. 20 DSGVO) besteht nicht, da die Verarbeitung nicht auf Einwilligung oder einem Vertrag beruht. Zur Ausübung dieser Rechte wenden Sie sich bitte an den Verantwortlichen unter der oben angegebenen E-Mail-Adresse. Ihr gesondertes Widerspruchsrecht nach Art. 21 DSGVO finden Sie weiter unten. Eine automatisierte Entscheidungsfindung einschließlich Profiling (Art. 22 DSGVO) findet nicht statt.',
