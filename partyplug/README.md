...
- **Transport is pluggable.** Talk to the Party Sockets relay
  (`PartyConnection`), with an optional P2P low-latency input path
  (`PartyFastlane`). (An `AirConsoleAdapter` exists upstream but is not vendored
  in this copy — AirConsole isn't used here yet.)
...
Conceptual model: slot 0 is always the display, slots 1..N are controllers.
Transport is pluggable behind one interface (here, `PartyConnection` over the
relay).
